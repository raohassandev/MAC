  ...
  (
    ...
    return createPortal(
      <div
        ref={ref}
        className={cn(
          'fixed z-50 min-w-[300px] max-w-md shadow-lg rounded-md border overflow-hidden transition-all duration-300',
          typeClasses[type],
      ...
      >
        <div className='flex p-4'>
          {showIcon && (
            <div className={cn('flex-shrink-0 mr-3', iconColors[type])}>
              {icon || typeIcon[type]}
            </div>
          )}
          ...
          {showCloseButton && (
            <button
              type='button'
              onClick={handleClose}
              className='flex-shrink-0 ml-1 text-gray-500 hover:text-gray-700'
              aria-label='Close'
            >
              <X size={16} />
            </button>
