  ...
  return (
    <Form.Group>
      <div className="flex items-center">
        <Form.Label htmlFor={htmlFor} required={required}>{label}</Form.Label>
...
const ParameterEditor: React.FC<ParameterEditorProps> = ({
  ...
  const isMultiRegister = ['INT32', 'UINT32', 'INT64', 'UINT64', 'FLOAT32', 'FLOAT', 'DOUBLE', 'FLOAT64', 'STRING', 'ASCII'].includes(parameter.dataType);
  const isNumericType = ['INT8', 'UINT8', 'INT16', 'UINT16', 'INT32', 'UINT32', 'INT64', 'UINT64', 'FLOAT32', 'FLOAT', 'DOUBLE', 'FLOAT64', 'BCD'].includes(parameter.dataType);
  const isBitType = ['BOOLEAN', 'BIT'].includes(parameter.dataType);
  const isStringType = ['STRING', 'ASCII'].includes(parameter.dataType);
  const isIntegerType = ['INT8', 'UINT8', 'INT16', 'UINT16', 'INT32', 'UINT32', 'INT64', 'UINT64'].includes(parameter.dataType);
  ...
  useEffect(() => {
    ...
    if (!isMultiRegister && ['ABCD', 'DCBA', 'BADC', 'CDAB'].includes(parameter.byteOrder)) {
      setParameter(prev => ({
        ...prev,
        byteOrder: 'AB'
      }));
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === 'checkbox') {
      parsedValue = checked;
    } else if (['scalingFactor', 'decimalPoint', 'registerIndex', 'wordCount', 'bitPosition', 'maxValue', 'minValue'].includes(name)) {
  ...
  const validateForm = (): boolean => {
    ...
    const isMultiRegister = ['INT32', 'UINT32', 'FLOAT32', 'FLOAT', 'INT64', 'UINT64', 'DOUBLE', 'FLOAT64', 'STRING', 'ASCII'].includes(parameter.dataType);
    if (isMultiRegister) {
                ...
                {errors.scalingEquation ? (
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    ...
                    <ul className="list-disc list-inside space-y-1 mt-1">
                      <li>Celsius to Fahrenheit: <code>x * 1.8 + 32</code></li>
                      <li>Scale by 0.1: <code>x * 0.1</code></li>
                      <li>Extract bits 8-15: <code>(x {'>>'} 8) & 0xFF</code></li>
                      <li>Convert negative: <code>x {'<'} 32768 ? x : x - 65536</code></li>
                    </ul>
                  </div>
                )}
              </FormFieldWithHelp>
            ...
            >
              ...
              {errors.bitmask ? (
                ...
                <div className="mt-1 text-xs text-gray-500">
                  <p>Common bitmasks:</p>
                  <ul className="list-disc list-inside space-y-1 mt-1">
                    <li><code>0xFF</code> - Low byte (bits 0-7)</li>
                    <li><code>0xFF00</code> - High byte (bits 8-15)</li>
                    <li><code>0xF0</code> - High nibble (bits 4-7)</li>
                    <li><code>0x0F</code> - Low nibble (bits 0-3)</li>
                    <li><code>0x8000</code> - Single bit (bit 15)</li>
                  </ul>
                </div>
              )}
            </FormFieldWithHelp>
