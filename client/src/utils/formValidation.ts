...
export const validateBasicInfo = (
  ...
  if (
    deviceData.make &&
    (deviceData.make.length < 2 || deviceData.make.length > 50)
  ) {
    addError(
      validation,
      'basicInfo',
  ...
  if (
    deviceData.model &&
    (deviceData.model.length < 1 || deviceData.model.length > 50)
  ) {
    addError(
      validation,
      'basicInfo',
...
export const validateConnection = (
  ...
  if (connectionType === 'tcp') {
    ...
    if (!deviceData.ip) {
      ...
      if (isValidIpv4) {
        ...
        !ipv6Pattern.test(deviceData.ip) &&
        !hostnamePattern.test(deviceData.ip)
      ) {
        addError(
          validation,
          'connection',
    ...
    if (isNaN(baudRate) || !validBaudRates.includes(baudRate)) {
      addError(
        ...
        'connection',
        'baudRate',
        'Baudrate must be one of the standard values: ' +
          validBaudRates.join(', ')
      );
    }
  ...
  if (deviceData.slaveId === undefined || deviceData.slaveId === '') {
    ...
    addError(
      ...
      'connection',
      'slaveId',
      'Slave ID must be a number between 0 and 247'
    );
  }
...
export const validateRegisterRange = (
  ...
  if (!newRegisterRange.rangeName) {
    ...
    newRegisterRange.rangeName.length < 2 ||
    newRegisterRange.rangeName.length > 50
  ) {
    addError(
      validation,
      'registers',
  ...
  const duplicateNameFound = existingRanges.some(
    (range, index) =>
      range.rangeName === newRegisterRange.rangeName &&
      (!isEditMode || index !== editingIndex)
  );
...
export const validateParameterConfig = (
  ...
  const duplicateNameFound = existingParams.some(
    (param) =>
      ...
      param.registerRange === paramConfig.registerRange
  );
  ...
  if (!paramConfig.registerRange) {
    ...
    if (!selectedRange) {
      addError(
        ...
        'parameters',
        'registerRange',
        'Selected register range does not exist'
      );
    } else {
      ...
      if (paramConfig.registerIndex < 0) {
        addError(
          ...
          'parameters',
          'registerIndex',
          'Register index cannot be negative'
        );
      } else if (
        paramConfig.registerIndex + requiredRegisters >
  ...
  if (paramConfig.decimalPoint !== undefined && (paramConfig.decimalPoint < 0 || paramConfig.decimalPoint > 10)) {
    addError(
      validation,
      'parameters',
      'decimalPoint',
      'Decimal points must be between 0 and 10'
    );
  }
...
export const validateDeviceForm = (
  ...
  registerRanges.forEach((range) => {
    const rangeParams = parameterConfigs.filter(
      (param) => param.registerRange === range.rangeName
    );
...
export const getFieldError = (
  ...
  for (const section of Object.keys(validation) as Array<
    keyof DeviceFormValidation
  >) {
