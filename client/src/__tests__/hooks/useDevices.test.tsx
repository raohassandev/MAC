...
describe('useDevices Hook', () => {
  ...
  beforeEach(() => {
    ...
    (deviceService.updateDevice as any).mockImplementation(
      (id, device) => Promise.resolve({ ...device, _id: id })
    );
    (deviceService.deleteDevice as any).mockResolvedValue(true);
    (deviceService.testConnection as any).mockResolvedValue({ success: true, message: 'Connected' });
